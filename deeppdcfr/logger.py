...
class Logger:
    def __init__(
        ...
        self.info("Logger is initialized")
        if self.folder:
            self.info(f"Save Folder is {self.folder.absolute()}")
    ...
    def _make_writers(self, folder: Optional[Path], writer_strings: List[str]) -> None:
        if self.redirect and folder:
            sys.stdout = open(folder / "stdout.log", "w")
            sys.stderr = open(folder / "stderr.log", "w")
        self.writers = []
    ...
    def record_max(self, key: str, value: Any) -> None:
        old_val, count = self.name_to_value[key], self.name_to_count[key]
        if count == 0:
            self.name_to_value[key] = value
        else:
            self.name_to_value[key] = max(old_val, value)
        self.name_to_count[key] = count + 1

    def record_min(self, key: str, value: Any) -> None:
        old_val, count = self.name_to_value[key], self.name_to_count[key]
        if count == 0:
            self.name_to_value[key] = value
        else:
            self.name_to_value[key] = min(old_val, value)
        self.name_to_count[key] = count + 1

    def dump(self, step: int = 0) -> None:
        self.record("timestamps", datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"))
    ...
    def _do_log(self, args, level) -> None:
        for writer in self.writers:
            if isinstance(writer, SeqWriter):
                writer.write_sequence(map(str, args), level)
...
class SeqWriter:
    def write_sequence(self, sequence: List, level) -> None:
        raise NotImplementedError
...
class CsvWriter(Writer):
    def __init__(self, folder: Path, csv_name: str = "data"):
        self.folder = folder
        self.data = pd.DataFrame(columns=["step"])
        self.csv_file = self.folder / f"{csv_name}.csv"
...
class MyLogger(logging.Logger):
    ...
    def findCaller(self, stack_info=False, stacklevel=1):
        ...
        while hasattr(f, "f_code"):
            ...
            if filename == str(Path(__file__).absolute()):
                f = f.f_back
                continue
            sinfo = None
            if stack_info:
    ...
    def write_sequence(self, sequence: List, level) -> None:
        ...
        for i, elem in enumerate(sequence):
            ...
            if i < len(sequence) - 1:  # add space unless this is the last one
                line += " "
        if level == DEBUG:
            self.writer.debug(line)
        if level == INFO:
            self.writer.info(line)
        if level == WARN:
            self.writer.warning(line)
        if level == ERROR:
            self.writer.error(line)
